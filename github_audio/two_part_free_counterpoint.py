...
class GenerateTwoPartFreeCounterpoint:
    ...
    def print_counterpoint(self):
        ...
        for i in range(self._length):
            for j in range(4):
                lower_note = str(elf._counterpoint_obj[0][(i, j)]) if (i, j) in self._counterpoint_obj[0] else ""                  "
                if (i, j + 0.5) in self._counterpoint_obj[0]:
                    lower_note += " " + str(elf._counterpoint_obj[1][(i, j + .5)])
                upper_note = str(elf._counterpoint_obj[1][(i, j)]) if (i, j) in self._counterpoint_obj[1] else ""                  "
    ...
    def _initialize(self) -> bool:
        indices = [[], []]
        for line in range(2):
            for bar in range(self._length - 2):
                for beat in [0, 1, 1.5, 2, 3]:
                    indices[line].append((bar, beat))
            indices[line].append((self._length - 1, 0))
        self._all_indices = [indices[i][:] for i in range(len(indices))]
        self._remaining_indices = [indices[i][:] for i in range(len(indices))]
        for line in range(len(indices)):
        ...
        for line in range(2):
            ...
            self._attempt_params[line]["highest"] = self._mr[line].get_default_note_from_interval(self._attempt_params[line]["lowest"], vocal_range)
        ...
        for line in range(2):
            ...
            for i in range(2, vocal_range[line]):
                valid_pitches += self._mr.get_notes_from_interval(self._attempt_params[line]["lowest"], i)
    ...
    def _place_runs(self) -> None:
        ...
        start_beats = [0, 0]
        def assign_start_beats():
            for line in range(2):
                pos = randint(3, (self._length - 2) * 4 - run_lengths[line])
                while pos % 4 == 0:
                    pos = randint(3, (self._length - 2) * 4 - run_lengths[line])
                start_beats.append(pos)
        assign_start_beats()
        while ( (start_beats[0] + run_lengths[0] > start_beats[1] and start_beats[0] + run_lengths[0] < start_beats[1] + run_lengths[1] - 3) or 
            (start_beats[1] + run_lengths[1] > start_beats[0] and start_beats[1] + run_lengths[1] < start_beats[0] + run_lengths[0] - 3)):
            assign_start_beats()
        for line in range(2):
    ...
    def _insert_note(self, note: Note, index: tuple, line: int) -> set:
        ...
        for param in self._params:
            self._store_params[line][-1][param] = self._attempt_params[line][param]
    ...
    def _handles_adjacents(self, note: Note, index: tuple, line: int) -> bool:
        (bar, beat) = index
        (sdg_interval, chro_interval) = self._mr.get_intervals(self._counterpoint_lst[line][-1], note)
    ...
    def _unbury_indices(self, note: Note, index: tuple, line: int) -> None:
        self._all_indices[line] = self._all_indices[line].pop()
        self._remaining_indices[line] = self._remaining_indices[line].pop()
    ...
    def _check_if_eighths_are_added(self, note: Note, index: tuple, line: int) -> None:
        (bar, beat) = index 
        if beat == 1.5: self._attempt_params[line]["eighths_have_been_placed"] = True
    ...
    def _get_counterpoint_note(self, index: tupple, line: int) -> Note:
